...
// Punto 1: 
// Sono stati inseriti in memoria i seguenti oggetti: devTeam, lead e i due oggetti dentro l'array testers che hanno proprietà name e skills. Se vogliamo contare solo gli oggetti "puri" siamo a 6, contando anche i due array skills e l'array testers (che tecnicamente sono degli oggetti, quantomeno in termini di type) il totale è 8. 
...
const createTester = (testerName)=>{
    ...
    const randomSkills = [];
    for(i=0; i<3; i++){ 
        const randomIndex = Math. floor( Math. random() * (totalSkills.length + 1) )
...
// Abbiamo aggiunto in totale 10 elementi infatti il nuovo oggetto devTeamCopy rispetto a devTeam contiene solo due oggetti in più: un oggetto tester e il suo array di skills. 
// Questo è vero perché al momento della definizione di devTeamCopy (riga 38) abbiamo creato un'effettiva copia dell'oggetto iniziale (invece che una reference all'oggetto di partenza come accade a riga 36 con la variabile teamLead e l'oggetto lead). Di conseguenza, tutte le modifiche che sono state apportate a devTeamCopy non influenzano devTeam perché sono due oggetti distinti. 
